    ...
    for i,sub_node in enumerate(subjects):
        ...
        def unenroll_button(sid = sid ,sub = sub):
            if st.button('Unenroll from this course',type='tertiary',width='stretch',icon=':material/delete_forever:',key=f"unenroll_{sid}"):
                ...
                st.toast(f"Uneroll form {sub['name']} Successfully!")
                st.rerun()
...
def student_screen():
    ...
    if show_registration:
        with st.container(border = True):
            ...
            st.subheader('Opitonal : Voice Enrollment')
            st.info('Enroll for voice only attendence')

            audio_data = None

            try:
                audio_data = st.audio_input('Record a short Phrase like I am Present,My name is Dev.')
            except Exception :
                ...
                if new_name:
                    with st.spinner('Creating profile..'):
                        ...
                        if encodings:
                            ...
                            st.error('Could not capture your facial features for registrations')                      
                else:
