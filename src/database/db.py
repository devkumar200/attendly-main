...
import bcrypt # for password hassing
...
def check_teacher_exists(username):
    # check for unique username , return false whern username is already taken
...
def teacher_login(username,password):
    resposne = supabase.table("teachers").select("*").eq("username",username).execute()
    if(resposne.data):
        teacher=resposne.data[0]
...
def delete_subject(subject_id,teacher_id):
    response = supabase.table("subjects").delete() \
